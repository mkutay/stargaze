...
const u_int64_t debruijn64 = 0x03f79d71b4cb0a89ull;
...
std::vector<u_int16_t> Board::get_moves() {
  ...
  for (int d = 0; d < 4; d++) {
    ...
    for (int m = 0; temp_rooks || temp_queens; m++) {
      ...
      while (temp) {
        u_int64_t ls1b = temp & -temp;
        int i = bit_scan_forward(ls1b);
        ret.emplace_back(create_move(i - (m + 1) * cardinal_moves_d[d], i, bool(temp_rooks_captures & ls1b) << 2));
        temp ^= ls1b;
      }
      temp = temp_queens;
      while (temp) {
        u_int64_t ls1b = temp & -temp;
        int i = bit_scan_forward(ls1b);
        ret.emplace_back(create_move(i - (m + 1) * cardinal_moves_d[d], i, bool(temp_queens_captures & ls1b) << 2));
        temp ^= ls1b;
      }
    }
  }
  ...
  for (int d = 0; d < 4; d++) {
    ...
    if (temp_king_cardinal) {
      ...
      ret.emplace_back(create_move(i - cardinal_moves_d[d], i, bool(temp_king_cardinal & temp_king_diagonal_captures) << 2));
    }
    if (temp_king_diagonal) {
      int i = bit_scan_forward(temp_king_diagonal);
      ret.emplace_back(create_move(i - diagonal_moves_d[d], i, bool(temp_king_diagonal & temp_king_cardinal_captures) << 2));
    }
  }
