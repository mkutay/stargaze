...
const auto mg_table = calculate_table(mg_pesto_table, mg_value);
const auto eg_table = calculate_table(eg_pesto_table, eg_value);

// gamephase_inc[piece] -- phase weight contribution per piece,
// colour-independent.
const std::array<int, 6> gamephase_inc = {0, 155, 305, 405, 1050, 0};
...
int Board::evaluate() const {
    ...
    for (Colour c : COLOURS) {
        for (Piece p : PIECES) {
            ...
            while (bb) {
                ...
                mg[ci] += mg_table[ci][pi][sq];
                eg[ci] += eg_table[ci][pi][sq];
                game_phase += gamephase_inc[pi];
            }
        }
    }
    ...
    int mg_score = mg[ti] - mg[nti];
    int eg_score = eg[ti] - eg[nti];
