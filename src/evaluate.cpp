...
const std::array<int, 64> mg_king_table = {
    ...
    -50, -16, 9,   8,   -15, 36,  12,  -54, 8,   -28, 28,  14,
};
...
const std::array<int, 6> mg_value = {82,  337,  365,
                                     477, 1025, 0}; // piece values
...
constexpr std::array<std::array<int, 64>, 12> calculate_mg_table() {
...
constexpr std::array<std::array<int, 64>, 12> calculate_eg_table() {
    std::array<std::array<int, 64>, 12> table{};
    for (int p = 0, pc = 0; p < 6; pc += 2, p++) {
        for (int i = 0; i < 64; i++) {
...
const std::array<int, 12> gamephase_inc = {0,   0,   155,  155,  305, 305,
