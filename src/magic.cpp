...
BitBoard slider_mask(Square sq, const std::array<int8_t, N> &moves) {
    BitBoard mask = 0;
    for (auto dir : moves) {
...
template <std::size_t N>
BitBoard attacks_slow(Square sq, BitBoard blockers,
...
void initialize_sliders(std::array<detail::Entry, 64> &entries,
                        std::array<BitBoard, TableSize> &table,
                        const std::array<int8_t, N> &moves,
                        const std::array<BitBoard, 64> &magics) {
    int offset = 0;
    for (Square sq = 0; sq < 64; sq++) {
        BitBoard mask = slider_mask(sq, moves);
        int bits = mask.count();
        ...
        for (int i = 0; i < occupancy_count; i++) {
            int index = static_cast<int>((occupancy.raw() * magic.raw()) >>
                                         (64 - bits));
            table[offset + index] = attacks_slow(sq, occupancy, moves);
            occupancy = mask & (occupancy.raw() - mask.raw());
        }

        offset += occupancy_count;
    }
    assert(offset == static_cast<int>(table.size()));
}

std::array<std::array<BitBoard, 64>, 64> generate_ray_between() {
...
MagicKeys::MagicKeys() : rook{}, bishop{}, rook_table{}, bishop_table{} {
    initialize_sliders(rook, rook_table, Mask::CARDINAL_MOVES, ROOK_MAGICS);
    initialize_sliders(bishop, bishop_table, Mask::DIAGONAL_MOVES,
                       BISHOP_MAGICS);
}

const MagicKeys keys{};

const std::array<std::array<BitBoard, 64>, 64> RAY_BETWEEN =
    generate_ray_between();
