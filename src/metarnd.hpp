/**
 * Meta programming random generator defined in `cr-lupin/metarand` with
 * paper.
 *
...
template <typename E> struct init {
    typedef E type;
};

template <typename Engine> struct Random {
...
template <typename R> struct Next {
    typedef typename eval<R>::type type;
    static const decltype(type::value) value = type::value;
    // static const uint_fast32_t value = linear_congruential_engine<R>::value;
    // //next random val
};

//--------------- linear_congruential_engine ---------------------

template <typename UIntType, UIntType seed = rs, UIntType a = 16807,
          UIntType c = 0, UIntType m = 2147483647>
...
struct eval<linear_congruential_engine<UIntType, seed, a, c, m>> {
    ...
    typedef linear_congruential_engine<UIntType, (a * seed + c) % m, a, c, m>
        type;
};
...
struct init<linear_congruential_engine<UIntType, seed, a, c, m>> {
    typedef
        typename eval<linear_congruential_engine<UIntType, seed, a, c, m>>::type
            type;
...
template <typename UIntType, UIntType a = 0, UIntType b = 1> struct fib_engine {
...
struct eval<fib_engine<UIntType, a, b>> {
    ...
    typedef fib_engine<UIntType, b, value> type;
};

// ------------------------ substract_with_carry --------------

template <typename UIntType, UIntType r, UIntType s, UIntType m,
          UIntType seed = rs>
struct substract_with_carry_engine {};

template <typename UIntType, UIntType r, UIntType s, UIntType m, UIntType res,
          UIntType... vals>
struct substract_with_carry_impl {
    static const UIntType value = res;

    template <unsigned int N, UIntType v, UIntType... vs> struct extract {
        const static UIntType value = extract<N - 1, vs...>::value;
    };

    template <UIntType v, UIntType... vs> struct extract<0, v, vs...> {
    ...
    // debug
    template <UIntType v, UIntType... vs> struct print_aux {
        static void print() {
            std::cout << v << " ";
            print_aux<vs...>::print();
        }
    ...
    template <UIntType v> struct print_aux<v> {
        static void print() { std::cout << v << std::endl; }
    };

    struct print {
        static void dump() { print_aux<vals...>::print(); }
    };
};
...
struct fillrnd {
    ...
    typedef typename fillrnd<UIntType, r, s, m, RND, cnt - 1, vals...,
                             RND::value>::type type;
};
...
struct fillrnd<UIntType, r, s, m, FillRnd, 0, vals...> {
    typedef substract_with_carry_impl<UIntType, r, s, m, 0, // 0 is dummy result
                                      vals...>
        type;
};

template <typename UIntType, UIntType r, UIntType s, UIntType m, UIntType seed>
...
struct eval<substract_with_carry_impl<UIntType, r, s, m, res, val, vals...>> {
    ...
    static const UIntType value =
        (X::template get<0>::value - X::template get<r - s>::value);
    // static const UIntType value = val + 1;
    typedef substract_with_carry_impl<UIntType, r, s, m, value, vals..., value>
        type;
};

// ------------------------------- Bernoulli

template <typename Engine, int N, int D, bool val = false> struct Bernoulli {
